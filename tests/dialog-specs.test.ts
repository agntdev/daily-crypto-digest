...
import { _resetStore } from "../src/lib/store";
...
describe("dialog specs (the publish gate replays these)", () => {
  it("every tests/specs/*.json spec passes against the real bot", async () => {
    ...
    // Reset the persistent store between specs so each spec starts with a clean
    // slate — prevents spec A's profile creation from polluting spec B's "new
    // user" /start test.
